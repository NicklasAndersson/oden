...
- **TAK: lösenord med `%` gick inte att använda.** Inställningarna byggs som en `ConfigParser`, vars standardinterpolation läser `%` som syntax — ett lösenord som innehåller t.ex. `%s` fick anslutningen att falla med `ValueError: invalid interpolation syntax`. Gällde både cert- och enrollment-lösenord
