...
if [ -f "./s7_watcher_linux" ]; then
    EXECUTABLE="./s7_watcher_linux"
else
    ...
    if ! command -v java &> /dev/null; then
        ...
        if [[ -z "$INSTALL_JAVA" || "$INSTALL_JAVA" =~ ^[Yy]$ ]]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                print_error "Installation failed. Please check apt output."
                exit 1
            fi
        else
            print_error "Java is required to continue. Exiting."
            exit 1
        fi
    else
        ...
        if [[ "$JAVA_MAJOR_VERSION" -lt 21 ]]; then
            ...
            echo "Please update: sudo apt install openjdk-21-jdk"
            exit 1
        else
...
if ! command -v qrencode &> /dev/null; then
    ...
    read -p "Install qrencode for QR code display? (Y/n): " INSTALL_QRENCODE
...
if $NEEDS_CONFIG; then
    ...
    # Get timezone
    CURRENT_TZ=$(timedatectl show --property=Timezone --value 2>/dev/null || echo "Europe/Stockholm")
